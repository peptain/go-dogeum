...
var (
	...
	// testAddr is the Dogeum address of the tester account.
...
func startEthService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *eth.Dogeum) {
	t.Helper()
...
func setupBlocks(t *testing.T, ethservice *eth.Dogeum, n int, parent *types.Block, callback func(parent *types.Block)) {
	api := NewConsensusAPI(ethservice)
	for i := 0; i < n; i++ {
		callback(parent)
