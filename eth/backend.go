...
import (
	...
	"github.com/dogeum-network-network/go-dogeum/consensus/beacon"
	"github.com/dogeum-network-network/go-dogeum/consensus/clique"
	...
	"github.com/dogeum-network-network/go-dogeum/core/state/pruner"
	...
	"github.com/dogeum-network-network/go-dogeum/eth/protocols/eth"
	"github.com/dogeum-network-network/go-dogeum/eth/protocols/snap"
	...
	"github.com/dogeum-network-network/go-dogeum/p2p/dnsdisc"
...
func New(stack *node.Node, config *ethconfig.Config) (*Dogeum, error) {
	...
	if !config.SkipBcVersionCheck {
		if bcVersion != nil && *bcVersion > core.BlockChainVersion {
			return nil, fmt.Errorf("database version is v%d, GDoge %s only supports v%d", *bcVersion, params.VersionWithMeta, core.BlockChainVersion)
...
func (s *Dogeum) Etherbase() (eb common.Address, err error) {
	s.lock.RLock()
...
func (s *Dogeum) SetEtherbase(etherbase common.Address) {
	s.lock.Lock()
	s.etherbase = etherbase
	s.lock.Unlock()
