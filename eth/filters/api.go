...
// information related to the Dogeum protocol such als blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(dogeum.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as dogeum.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria dogeum.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(dogeum.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
