...
// the process. This is usually the case when GDoge is shutting down and some events
...
//
// Opposed to the original Dogeum block synchronization which is trustless (and
...
// not from within (vs. legacy Dogeum sync).
//
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but GDoge is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until GDoge requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
