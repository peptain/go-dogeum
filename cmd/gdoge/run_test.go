...
import (
	...
	"github.com/dogeum-network-network/go-dogeum/internal/cmdtest"
	"github.com/dogeum-network-network/go-dogeum/rpc"
)

type testgdoge struct {
	*cmdtest.TestCmd
...
func init() {
	// Run the app if we've been exec'd as "gdoge-test" in runGDoge.
	reexec.Register("gdoge-test", func() {
...
// spawns gdoge with the given command line args. If the args don't set --datadir, the
// child g gets a temporary data directory.
func runGDoge(t *testing.T, args ...string) *testgdoge {
	tt := &testgdoge{}
	...
	// Boot "gdoge". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("gdoge-test", args...)
