...
import (
	...
	"github.com/dogeum-network-network/go-dogeum/accounts/external"
	...
	"github.com/dogeum-network-network/go-dogeum/accounts/usbwallet"
...
func loadConfig(file string, cfg *gdogeConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gdoge.ipc"
	return cfg
}

// makeConfigNode loads gdoge configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gdogeConfig) {
	// Load defaults.
	cfg := gdogeConfig{
...
// makeFullNode loads gdoge configuration and creates the Dogeum backend.
func makeFullNode(ctx *cli.Context) (*node.Node, ethapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `gdoge db freezer-migrate`.")
		}
	}
	...
	// Add the Dogeum Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *gdogeConfig) {
