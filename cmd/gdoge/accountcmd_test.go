...
`
	...
	{
		gdoge := runGDoge(t, "account", "list", "--datadir", datadir)
		gdoge.Expect(want)
		gdoge.ExpectExit()
	}
	{
		gdoge := runGDoge(t, "--datadir", datadir, "account", "list")
		gdoge.Expect(want)
		gdoge.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gdoge := runGDoge(t, "account", "new", "--lightkdf")
	defer gdoge.ExpectExit()
	gdoge.Expect(`
...
func TestAccountHelp(t *testing.T) {
	gdoge := runGDoge(t, "account", "-h")
	gdoge.WaitExit()
	if have, want := gdoge.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gdoge = runGDoge(t, "account", "import", "-h")
	gdoge.WaitExit()
	if have, want := gdoge.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gdoge := runGDoge(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gdoge.ExpectExit()
	gdoge.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gdoge := runGDoge(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gdoge := runGDoge(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gdoge.ExpectExit()
	gdoge.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gdoge.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gdoge := runMinimalGDoge(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gdoge.StderrText(), m) {
...
// https://github.com/dogeum-network-network/go-dogeum/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gdoge.SetTemplateFunc("keypath", func(file string) string {
