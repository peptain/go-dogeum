...
// Package utils contains internal helper functions for go-dogeum commands.
package utils
...
import (
	...
	"github.com/dogeum-network-network/go-dogeum/common/fdlimit"
	"github.com/dogeum-network-network/go-dogeum/consensus"
	"github.com/dogeum-network-network/go-dogeum/consensus/ethash"
	"github.com/dogeum-network-network/go-dogeum/core"
	"github.com/dogeum-network-network/go-dogeum/core/rawdb"
	"github.com/dogeum-network-network/go-dogeum/core/vm"
	"github.com/dogeum-network-network/go-dogeum/crypto"
	"github.com/dogeum-network-network/go-dogeum/eth"
	ethcatalyst "github.com/dogeum-network-network/go-dogeum/eth/catalyst"
	"github.com/dogeum-network-network/go-dogeum/eth/downloader"
	"github.com/dogeum-network-network/go-dogeum/eth/ethconfig"
	"github.com/dogeum-network-network/go-dogeum/eth/filters"
	"github.com/dogeum-network-network/go-dogeum/eth/gasprice"
	"github.com/dogeum-network-network/go-dogeum/eth/tracers"
	"github.com/dogeum-network-network/go-dogeum/ethdb"
	"github.com/dogeum-network-network/go-dogeum/ethdb/remotedb"
	"github.com/dogeum-network-network/go-dogeum/ethstats"
	"github.com/dogeum-network-network/go-dogeum/graphql"
	"github.com/dogeum-network-network/go-dogeum/internal/ethapi"
	"github.com/dogeum-network-network/go-dogeum/internal/flags"
	"github.com/dogeum-network-network/go-dogeum/les"
	lescatalyst "github.com/dogeum-network-network/go-dogeum/les/catalyst"
	"github.com/dogeum-network-network/go-dogeum/log"
	"github.com/dogeum-network-network/go-dogeum/metrics"
	"github.com/dogeum-network-network/go-dogeum/metrics/exp"
	"github.com/dogeum-network-network/go-dogeum/metrics/influxdb"
	"github.com/dogeum-network-network/go-dogeum/miner"
	"github.com/dogeum-network-network/go-dogeum/node"
	"github.com/dogeum-network-network/go-dogeum/p2p"
	"github.com/dogeum-network-network/go-dogeum/p2p/enode"
	"github.com/dogeum-network-network/go-dogeum/p2p/nat"
	"github.com/dogeum-network-network/go-dogeum/p2p/netutil"
...
var (
	...
	RPCGlobalTxFeeCapFlag = &cli.Float64Flag{
		Name:     "rpc.txfeecap",
		Usage:    "Sets a cap on transaction fee (in doge) that can be sent via the RPC APIs (0 = no cap)",
	...
	IgnoreLegacyReceiptsFlag = &cli.BoolFlag{
		...
		Usage:    "GDoge will start up even if there are legacy receipts in freezer",
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(RopstenFlag.Name) {
			// Maintain compatibility with older GDoge configurations storing the
...
// for GDoge and returns half of the allowance to assign to the database.
...
func MakeAddress(ks *keystore.KeyStore, account string) (accounts.Account, error) {
	...
	log.Warn("Please use explicit addresses! (can search via `gdoge account list`)")
	log.Warn("-------------------------------------------------------------------")
...
// RegisterEthService adds an Dogeum client to the stack.
// The second return value is the full node instance, which may be nil if the
// node is running as a light client.
func RegisterEthService(stack *node.Node, cfg *ethconfig.Config) (ethapi.Backend, *eth.Dogeum) {
	if cfg.SyncMode == downloader.LightSync {
		backend, err := les.New(stack, cfg)
		if err != nil {
			Fatalf("Failed to register the Dogeum service: %v", err)
		}
...
// RegisterEthStatsService configures the Dogeum Stats daemon and adds it to the node.
func RegisterEthStatsService(stack *node.Node, backend ethapi.Backend, url string) {
	if err := ethstats.New(stack, backend, backend.Engine(), url); err != nil {
		Fatalf("Failed to register the Dogeum Stats service: %v", err)
	}
}
...
func SetupMetrics(ctx *cli.Context) {
	if metrics.Enabled {
		...
		if enableExport {
			...
			go influxdb.InfluxDBWithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, database, username, password, "gdoge.", tagsMap)
		} else if enableExportV2 {
			tagsMap := SplitTagsFlag(ctx.String(MetricsInfluxDBTagsFlag.Name))

			log.Info("Enabling metrics export to InfluxDB (v2)")

			go influxdb.InfluxDBV2WithTags(metrics.DefaultRegistry, 10*time.Second, endpoint, token, bucket, organization, "gdoge.", tagsMap)
		}
