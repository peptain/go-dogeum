...
import (
	...
	"github.com/dogeum-network-network/go-dogeum/internal/utesting"
...
func TestEthSuite(t *testing.T) {
	gdoge, err := runGDoge()
	if err != nil {
		t.Fatalf("could not run gdoge: %v", err)
	}
	defer gdoge.Close()

	suite, err := NewSuite(gdoge.Server().Self(), fullchainFile, genesisFile)
	if err != nil {
...
// runGDoge creates and starts a gdoge node
func runGDoge() (*node.Node, error) {
...
func setupGDoge(stack *node.Node) error {
