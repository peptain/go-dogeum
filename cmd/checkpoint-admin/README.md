...
*For a more detailed introduction to checkpoint, please see the LES [spec](https://github.com/dogeum-network-network/devp2p/blob/master/caps/les.md).*
...
```shell
go get github.com/dogeum-network-network/go-dogeum/cmd/checkpoint-admin
```
...
It is worth noting that checkpoint-admin only supports clef as a signer for transactions and plain text(checkpoint). For more clef usage, please see the clef [tutorial](https://gdoge.ethereum.org/docs/clef/tutorial) .
...
*It is worth noting that the connected GDoge node can be a fullnode or a light client. If it is fullnode, you must enable the LES protocol. E.G. add `--light.serv 50` to the startup command line flags*.
...
*CHECKPOINT_HASH is obtained based on this [calculation method](https://github.com/dogeum-network-network/go-dogeum/blob/master/params/config.go#L251).*
...
* Start gdoge with the modified configuration file
