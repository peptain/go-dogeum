...
import (
	...
	"github.com/dogeum-network-network/go-dogeum/les/utils"
...
type PeerInfo struct {
	Version    int      `json:"version"`    // Dogeum protocol version negotiated
	...
	return p.handshake(td, head, headNum, genesis, forkID, forkFilter, func(lists *keyValueList) {
		...
		if !server.config.UltraLightOnlyAnnounce {
			...
			*lists = (*lists).add("serveStateSince", uint64(0))
			...
			// If local dogeum node is running in archive mode, advertise ourselves we have
...
// participating in the Light Dogeum sub-protocol.
