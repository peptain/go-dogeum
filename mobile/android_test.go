...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", GDoge.LightScryptN, GDoge.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.dogeum", new NodeConfig());
			node.start();
			...
			DogeumClient ec = node.getDogeumClient();
	...
	// signatures too. Regression test for go-dogeum issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/dogeum/gdogetest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for GDoge and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.dogeum", "github.com/dogeum-network-network/go-dogeum/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "gdoge.aar"), "gdoge.aar")

	if err = os.WriteFile(filepath.Join("src", "androidTest", "java", "org", "dogeum", "gdogetest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
