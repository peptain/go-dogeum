...
// Package gdogeclient provides an RPC client for gdoge-specific APIs.
package gdogeclient
...
// Client is a wrapper around rpc.Client that implements gdoge-specific functionality.
//
// If you want to use the standardized Dogeum RPC functionality, use ethclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg dogeum.CallMsg) (*types.AccessList, uint64, string, error) {
...
func (ec *Client) CallContract(ctx context.Context, msg dogeum.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a gdoge node.
...
// MemStats retrieves the current memory stats from a gdoge node.
...
// GetNodeInfo retrieves the node info of a gdoge node.
