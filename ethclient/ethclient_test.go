...
// Verify that Client implements the dogeum interfaces.
var (
	_ = dogeum.ChainReader(&Client{})
	_ = dogeum.TransactionReader(&Client{})
	_ = dogeum.ChainStateReader(&Client{})
	_ = dogeum.ChainSyncReader(&Client{})
	_ = dogeum.ContractCaller(&Client{})
	_ = dogeum.GasEstimator(&Client{})
	_ = dogeum.GasPricer(&Client{})
	_ = dogeum.LogFilterer(&Client{})
	_ = dogeum.PendingStateReader(&Client{})
	// _ = dogeum.PendingStateEventer(&Client{})
	_ = dogeum.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Dogeum Service
	config := &ethconfig.Config{Genesis: genesis}
	config.Ethash.PowMode = ethash.ModeFake
	ethservice, err := eth.New(n, config)
	if err != nil {
		t.Fatalf("can't create new dogeum service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == dogeum.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != dogeum.NotFound {
		t.Fatal("error should be dogeum.NotFound")
	}
}
