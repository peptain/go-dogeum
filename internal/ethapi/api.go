...
import (
	...
	"github.com/dogeum-network-network/go-dogeum/accounts"
	"github.com/dogeum-network-network/go-dogeum/accounts/abi"
	"github.com/dogeum-network-network/go-dogeum/accounts/keystore"
	"github.com/dogeum-network-network/go-dogeum/accounts/scwallet"
	"github.com/dogeum-network-network/go-dogeum/common"
	"github.com/dogeum-network-network/go-dogeum/common/hexutil"
	"github.com/dogeum-network-network/go-dogeum/common/math"
	"github.com/dogeum-network-network/go-dogeum/consensus/ethash"
	"github.com/dogeum-network-network/go-dogeum/consensus/misc"
	"github.com/dogeum-network-network/go-dogeum/core"
	"github.com/dogeum-network-network/go-dogeum/core/state"
	"github.com/dogeum-network-network/go-dogeum/core/types"
	"github.com/dogeum-network-network/go-dogeum/core/vm"
	"github.com/dogeum-network-network/go-dogeum/crypto"
	"github.com/dogeum-network-network/go-dogeum/eth/tracers/logger"
	...
	"github.com/dogeum-network-network/go-dogeum/rlp"
	"github.com/dogeum-network-network/go-dogeum/rpc"
	"github.com/tyler-smith/go-bip39"
)

// DogeumAPI provides an API to access Dogeum related information.
type DogeumAPI struct {
	b Backend
}

// NewDogeumAPI creates a new Dogeum protocol API.
func NewDogeumAPI(b Backend) *DogeumAPI {
	return &DogeumAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *DogeumAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *DogeumAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *DogeumAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *DogeumAPI) Syncing() (interface{}, error) {
...
// DogeumAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type DogeumAccountAPI struct {
	am *accounts.Manager
}

// NewDogeumAccountAPI creates a new DogeumAccountAPI.
func NewDogeumAccountAPI(am *accounts.Manager) *DogeumAccountAPI {
	return &DogeumAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *DogeumAccountAPI) Accounts() []common.Address {
...
// Sign calculates an Dogeum ECDSA signature for:
// keccak256("\x19Dogeum Signed Message:\n" + len(message) + message))
//
...
//
// https://github.com/dogeum-network-network/go-dogeum/wiki/Management-APIs#personal_sign
...
// the address of:
// hash = keccak256("\x19Dogeum Signed Message:\n"${message length}${message})
// addr = ecrecover(hash, signature)
//
// Note, the signature must conform to the secp256k1 curve R, S and V values, where
// the V value must be 27 or 28 for legacy reasons.
//
// https://github.com/dogeum-network-network/go-dogeum/wiki/Management-APIs#personal_ecRecover
func (s *PersonalAccountAPI) EcRecover(ctx context.Context, data, sig hexutil.Bytes) (common.Address, error) {
	if len(sig) != crypto.SignatureLength {
		return common.Address{}, fmt.Errorf("signature must be %d bytes long", crypto.SignatureLength)
	}
	if sig[crypto.RecoveryIDOffset] != 27 && sig[crypto.RecoveryIDOffset] != 28 {
		return common.Address{}, fmt.Errorf("invalid Dogeum signature (V is not 27 or 28)")
	}
...
// Unpair deletes a pairing between wallet and gdoge.
...
// BlockChainAPI provides an API to access Dogeum blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Dogeum blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Dogeum chain config.
//
...
// See: https://github.com/dogeum-network-network/wiki/wiki/JSON-RPC-Error-Codes-Improvement-Proposal
...
//
// https://github.com/dogeum-network-network/wiki/wiki/JSON-RPC#eth_sign
...
// DebugAPI is the collection of Dogeum APIs exposed over the debugging
// namespace.
...
// Version returns the current dogeum protocol version.
...
func checkTxFee(gasPrice *big.Int, gas uint64, cap float64) error {
	...
	feeEth := new(big.Float).Quo(new(big.Float).SetInt(new(big.Int).Mul(gasPrice, new(big.Int).SetUint64(gas))), new(big.Float).SetInt(big.NewInt(params.Doge)))
	feeFloat, _ := feeEth.Float64()
	if feeFloat > cap {
		return fmt.Errorf("tx fee (%.2f doge) exceeds the configured cap (%.2f doge)", feeFloat, cap)
	}
	return nil
}
