...
import (
	...
	"github.com/dogeum-network-network/go-dogeum/core/beacon"
...
type ethNode struct {
	...
	ethBackend *eth.Dogeum
	lapi       *lescatalyst.ConsensusAPI
	lesBackend *les.LightDogeum
}
...
func makeFullNode(genesis *core.Genesis) (*node.Node, *eth.Dogeum, *ethcatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Dogeum node
	...
	// Create the node and configure a full Dogeum node on it
...
func makeLightNode(genesis *core.Genesis) (*node.Node, *les.LightDogeum, *lescatalyst.ConsensusAPI, error) {
	// Define the basic configurations for the Dogeum node
	datadir, _ := os.MkdirTemp("", "")
