...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gdoge is "gdoge". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "GDoge", keep that.
	if name == "gdoge" || name == "gdoge-testnet" {
		name = "GDoge"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gdoge" instances.
var isOldGDogeResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gdoge 1.4 are used if they exist.
	if warn, isOld := isOldGDogeResource[path]; isOld {
		oldpath := ""
		if c.name() == "gdoge" {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGDogeResourceWarning, "Using deprecated resource file %s, please move this file to the 'gdoge' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func getKeyStoreDir(conf *Config) (string, bool, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = os.MkdirTemp("", "go-dogeum-keystore")
		isEphemeral = true
	}
